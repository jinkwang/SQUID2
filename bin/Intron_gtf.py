...
def transcript_intron(gtf_file_list):
    ...
    for gtf_line in gtf_file_list:
        ...
        if len(gtf_list) < 9 or gtf_list[2] != 'exon':
            continue
        transcript_id = re.sub('.*transcript_id "|\".*', '', gtf_list[8])
        t_strand = gtf_list[6]
        if strand == 'unstrand':
        ...
        if trans.has_key(transcript_id):
            if int(gtf_list[3]) < trans[transcript_id][0]:
                trans[transcript_id][0] = int(gtf_list[3])
            if int(gtf_list[4]) > trans[transcript_id][1]:
                trans[transcript_id][1] = int(gtf_list[4])
        else:
            trans[transcript_id] = [int(gtf_list[3]), int(gtf_list[4]), gtf_list[0], t_strand]
    ...
    for t in trans:
        ...
        for i in xrange(index_s, index_e + 1):
            pos[(trans[t][2], trans[t][3], i)].append(t)
    intron_file = open("%s/Intron_%s" % (path, gtf))
    intron = defaultdict(lambda: [])
    for line in intron_file:
        intron_line_list = line.strip().split('\t')
        key = "%s_%s_%s" % (intron_line_list[0], intron_line_list[3], intron_line_list[4])
        index_s = int(intron_line_list[3]) / bin
        index_e = int(intron_line_list[4]) / bin
        intron_strand = intron_line_list[6]
        if strand == 'unstrand':
            intron_strand = '*'
        for i in xrange(index_s, index_e + 1):
            if (intron_line_list[0], intron_strand, i) in pos:
                for t in pos[(intron_line_list[0], intron_strand, i)]:
                    if int(intron_line_list[3]) >= trans[t][0] and int(intron_line_list[4]) <= trans[t][1]:
                        intron[key].append(t)
    intron_file.close()
...
def attribute_intron(gtf_file_list):
    ...
    for line in gtf_file_list:
        ...
        key = "%s_%s_%s" % (gtf_list[0], gtf_list[3], gtf_list[4])
        gene_id = re.sub('.*gene_id "|\".*', '', gtf_list[8])
        if key in intron:
        ...
        for i in range(index_s, index_e + 1):
            if (sp[0], exon_strand, i) in pos:
                for j in pos[sp[0], exon_strand, i]:
                    if (int(sp[3]) >= intron[j][3] and int(sp[3]) < intron[j][4]):
                        intron[j][0] = "false"
                    if ((int(sp[4]) > intron[j][3]) and (int(sp[4]) <= intron[j][4])):
                        intron[j][0] = "false"
    exon_file.close()
            ...
            if (sp[0], intron_strand, i) in pos:
                for j in pos[sp[0], intron_strand, i]:
                    if (int(sp[3]) == intron[j][3] and int(sp[4]) == intron[j][4]):
                        continue
                    if (int(sp[3]) == intron[j][3] or int(sp[4]) == intron[j][4]):
                        intron[j][1] = "false"
                    if (int(sp[3]) < intron[j][3] and int(sp[4]) > intron[j][3]):
                        intron[j][1] = "false"
                    if ((int(sp[3]) > intron[j][3]) and (int(sp[3]) < intron[j][4])):
                        intron[j][1] = "false"
